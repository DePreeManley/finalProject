...
xhr.onload = function () {
  ...
  if (xhr.status === 200) {
    ...
    for (let index = 0; index < responseObject.images.length; index++) {
      ...
      newCard += '<div class=card>'
      newCard += '<h2>' + info.Events[0].title + '</h2>'
      newCard += '<img src="' + responseObject.images[index].event + '"/>'
      newCard += '<button class="BTTN">'
      newCard += '<img src="' + responseObject.icons[index].Plus + '"class="icon"/>'
      newCard += '</button>'
      newCard += '<div class="secondary_card">'
      newCard += '<p>' + info.Events[0].name + '</p>'
      newCard += '<p>' + info.Events[0].date + '</p>'
      newCard += '<p>' + info.Events[0].adress + '</p>'
      newCard += '<p>' + info.Events[0].Text + '</p>'
      newCard += '<a href="https://www.experiencecolumbus.com/events/">' + info.Events[0].link + '</a>'
      newCard += '</div>'
      newCard += '</div>'

      newCard += '<div class=card>'
      newCard += '<h2>' + info.PlacesToSee[0].title + '</h2>'
      newCard += '<img src="' + responseObject.images[index].rock + '"/>'
      newCard += '<button class="BTTN">'
      newCard += '<img src="' + responseObject.icons[index].Plus + '"class="icon"/>'
      newCard += '</button>'
      newCard += '<div class="secondary_card">'
      newCard += '<p>' + info.PlacesToSee[0].name + '</p>'
      newCard += '<p>' + info.PlacesToSee[0].adress + '</p>'
      newCard += '<p>' + info.PlacesToSee[0].Text + '</p>'
      newCard += '<a href="https://www.experiencecolumbus.com/things-to-do/">' + info.PlacesToSee[0].link + '</a>'
      newCard += '</div>'
      newCard += '</div>'
